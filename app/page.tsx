    ...
    <div style={{ minHeight:'100vh', background:'#0A0A0A', color:'#F5F5F0' }}>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn-ghost:hover { background: rgba(201,168,76,0.1) !important; border-color: rgba(201,168,76,0.6) !important; }
      ...
      {/* BANNER */}
      {bannerVisible && (
        <div style={{ background:'linear-gradient(90deg, #8B6914, #C9A84C, #8B6914)', color:'#000', padding:'10px 2rem', textAlign:'center', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center', gap:'1rem', fontWeight:500, animation:'slideDown 0.5s ease' }}>
          <span>✦ Spedizione gratuita sopra €50 — codice <strong>GABRY10</strong> per il 10% di sconto ✦</span>
          <button onClick={() => setBannerVisible(false)} style={{ background:'none', border:'none', color:'#000', cursor:'pointer', fontSize:16, opacity:0.6 }}>✕</button>
        </div>
      )}
      ...
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1.25rem 2.5rem', borderBottom:'1px solid rgba(201,168,76,0.1)', position:'sticky', top:0, background:'rgba(10,10,10,0.95)', backdropFilter:'blur(20px)', zIndex:50 }}>
        <div onClick={() => setSelectedProduct(null)} style={{ fontSize:22, fontWeight:700, cursor:'pointer', fontFamily:'Playfair Display, serif', color:'#C9A84C', letterSpacing:'0.5px' }}>GabryShopss</div>
        <div style={{ display:'flex', gap:'2rem', alignItems:'center' }}>
          {[['Prodotti','/'],['Chi siamo','/chi-siamo'],['Contattaci','/contatti']].map(([label, href]) => (
            <a key={label} href={href} style={{ fontSize:14, color:'#888880', textDecoration:'none', transition:'color 0.2s', letterSpacing:'0.3px' }}
              onMouseEnter={e => (e.currentTarget.style.color='#C9A84C')}
              onMouseLeave={e => (e.currentTarget.style.color='#888880')}>{label}</a>
          ))}
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:16 }}>
          ...
          <button onClick={() => setCartOpen(true)} style={{ position:'relative', background:'none', border:'1px solid rgba(201,168,76,0.3)', borderRadius:10, padding:'8px 12px', cursor:'pointer', fontSize:18, color:'#C9A84C', transition:'all 0.2s' }}
      ...
      {selectedProduct ? (
        <div style={{ maxWidth:1100, margin:'0 auto', padding:'2.5rem 2rem', animation:'fadeUp 0.4s ease' }}>
          <button onClick={() => setSelectedProduct(null)} style={{ background:'none', border:'none', fontSize:14, color:'#888880', cursor:'pointer', marginBottom:'2rem', display:'flex', alignItems:'center', gap:8, transition:'color 0.2s' }}
            onMouseEnter={e => (e.currentTarget.style.color='#C9A84C')}
            onMouseLeave={e => (e.currentTarget.style.color='#888880')}>← Torna ai prodotti</button>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'4rem', marginBottom:'4rem' }}>
            <div>
              <div onClick={() => setZoomedImg(selectedProduct.images[selectedImg])} style={{ position:'relative', height:420, borderRadius:16, overflow:'hidden', background:'#141414', marginBottom:12, cursor:'zoom-in', border:'1px solid rgba(201,168,76,0.1)' }}>
                ...
                <div style={{ position:'absolute', top:12, right:12, background:'rgba(0,0,0,0.7)', color:'#C9A84C', padding:'6px 12px', borderRadius:20, fontSize:11, border:'1px solid rgba(201,168,76,0.3)', backdropFilter:'blur(10px)' }}>🔍 Zoom</div>
              </div>
            ...
            <div style={{ display:'flex', flexDirection:'column', justifyContent:'center' }}>
              ...
              {avgStelle && (
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:16 }}>
                  <span style={{ color:'#C9A84C', fontSize:16 }}>{'★'.repeat(Math.round(Number(avgStelle)))}</span>
                  <span style={{ fontSize:13, color:'#888880' }}>{avgStelle} ({reviews.length} recensioni)</span>
                </div>
              )}
              ...
              {selectedProduct.type === 'digitale' && <p style={{ fontSize:13, color:'#C9A84C', marginBottom:20 }}>✅ Download immediato · Spedizione gratuita</p>}
              ...
              <div style={{ display:'flex', gap:10, marginBottom:10 }}>
                ...
                <button className="btn-ghost" onClick={() => { const url = window.location.href; navigator.share ? navigator.share({ title: selectedProduct.name, url }) : navigator.clipboard.writeText(url).then(() => setToast('🔗 Link copiato!')) }} style={{ border:'none', padding:'15px 18px', borderRadius:24, fontSize:18, cursor:'pointer' }}>📤</button>
              </div>
              <button className="btn-ghost" onClick={() => { addToCart(selectedProduct); setCartOpen(true); }} style={{ border:'none', padding:'15px', borderRadius:24, fontSize:15, cursor:'pointer', fontWeight:500 }}>
                Acquista subito
              </button>
            </div>
          </div>
          ...
          <div style={{ borderTop:'1px solid rgba(201,168,76,0.15)', paddingTop:'3rem', marginBottom:'3rem' }}>
            <h2 style={{ fontSize:26, fontWeight:700, marginBottom:'2rem', fontFamily:'Playfair Display, serif', color:'#F5F5F0' }}>Recensioni {reviews.length > 0 && <span style={{ color:'#C9A84C' }}>({reviews.length})</span>}</h2>
            <div style={{ background:'#141414', border:'1px solid rgba(201,168,76,0.15)', borderRadius:16, padding:'1.5rem', marginBottom:'2rem' }}>
              <h3 style={{ fontSize:15, fontWeight:600, marginBottom:'1rem', color:'#F5F5F0' }}>Scrivi una recensione</h3>
              ...
              <div style={{ marginBottom:12 }}>
                <div style={{ fontSize:13, color:'#888880', marginBottom:8 }}>Valutazione</div>
                <div style={{ display:'flex', gap:4 }}>
                  {[1,2,3,4,5].map(s => (
                    <button key={s} onClick={() => setNewReview({...newReview, stelle:s})} style={{ background:'none', border:'none', fontSize:26, cursor:'pointer', color: s <= newReview.stelle ? '#C9A84C' : '#333', transition:'color 0.2s' }}>★</button>
                  ))}
                </div>
              </div>
            ...
            {reviews.length === 0 ? (
              <div style={{ textAlign:'center', padding:'2rem', color:'#444440', fontSize:14 }}>Nessuna recensione ancora — sii il primo!</div>
            ) : reviews.map(r => (
              <div key={r.id} style={{ background:'#141414', border:'1px solid rgba(201,168,76,0.1)', borderRadius:12, padding:'1.25rem', marginBottom:10 }}>
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                  ...
                  <span style={{ fontSize:12, color:'#444440' }}>{new Date(r.created_at).toLocaleDateString('it-IT')}</span>
                </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(220px, 1fr))', gap:'1rem' }}>
              {products.filter(p => p.id !== selectedProduct.id).slice(0,3).map(p => (
                <div key={p.id} className="card" onClick={() => { setSelectedProduct(p); setSelectedImg(0); window.scrollTo({top:0,behavior:'smooth'}); }} style={{ background:'#141414', borderRadius:12, overflow:'hidden', cursor:'pointer' }}>
                  <div style={{ position:'relative', height:140, background:'#1C1C1C' }}>
        ...
        <>
          {/* HERO */}
          <div style={{ textAlign:'center', padding:'7rem 2rem 4rem', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ fontSize:12, color:'#C9A84C', letterSpacing:'4px', textTransform:'uppercase', marginBottom:'1.5rem', fontWeight:500, animation:'fadeUp 0.6s ease' }}>Premium Auto Care</div>
            <h1 style={{ fontSize:64, fontWeight:700, letterSpacing:'-2px', marginBottom:'1.5rem', lineHeight:1.05, fontFamily:'Playfair Display, serif', animation:'fadeUp 0.6s ease 0.1s both' }}>
              Prodotti Auto<br /><span style={{ color:'#C9A84C' }}>di Qualità.</span>
            </h1>
            <p style={{ fontSize:17, color:'#888880', maxWidth:480, margin:'0 auto 2.5rem', lineHeight:1.7, animation:'fadeUp 0.6s ease 0.2s both' }}>
              Tutto quello che serve per la cura e la protezione della tua auto. Selezione professionale.
            </p>
            <button className="btn-gold" onClick={() => document.getElementById('products')?.scrollIntoView({behavior:'smooth'})} style={{ border:'none', padding:'14px 36px', borderRadius:30, fontSize:15, cursor:'pointer', fontWeight:600, animation:'fadeUp 0.6s ease 0.3s both' }}>
              Scopri i prodotti
            </button>
          </div>

          {/* FILTRI */}
          <div style={{ display:'flex', gap:8, justifyContent:'center', padding:'0 2rem 2rem' }}>
            {['tutti','fisico','digitale'].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding:'8px 22px', borderRadius:24, border: filter===f ? 'none' : '1px solid rgba(201,168,76,0.2)', background: filter===f ? 'linear-gradient(135deg, #C9A84C, #E8C97A)' : 'transparent', color: filter===f ? '#000' : '#888880', fontSize:13, cursor:'pointer', fontWeight: filter===f ? 600 : 400, transition:'all 0.2s' }}>
          ...
          {/* GRIGLIA */}
          {loading ? (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:'1.5rem', padding:'0 2rem 4rem', maxWidth:1200, margin:'0 auto' }}>
              {[1,2,3].map(i => (
                <div key={i} style={{ borderRadius:16, height:380, background:'linear-gradient(90deg, #141414 25%, #1C1C1C 50%, #141414 75%)', backgroundSize:'200% 100%', animation:'shimmer 1.5s infinite' }} />
              ))}
            </div>
          ) : (
            <div id="products" style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(320px, 1fr))', gap:'1.5rem', padding:'0 2rem 5rem', maxWidth:1200, margin:'0 auto' }}>
              {filtered.map((p, idx) => {
                const inCart = cart.find(c => c.id === p.id)
                const badges = ['🔥 Bestseller', '⭐ Top qualità', '🆕 Novità']
                return (
                  <div key={p.id} className="card" style={{ background:'#141414', borderRadius:16, overflow:'hidden', cursor:'pointer', animation:`fadeUp 0.5s ease ${idx*0.1}s both` }} onClick={() => { setSelectedProduct(p); setSelectedImg(0); }}>
                    <div style={{ position:'relative', height:260, background:'#1C1C1C' }}>
                      <Image src={p.image} alt={p.name} fill style={{ objectFit:'cover', transition:'transform 0.4s' }} />
                      <div style={{ position:'absolute', top:14, left:14, background:'rgba(0,0,0,0.8)', color:'#C9A84C', padding:'5px 12px', borderRadius:20, fontSize:11, fontWeight:500, border:'1px solid rgba(201,168,76,0.3)', backdropFilter:'blur(10px)' }}>{badges[idx % 3]}</div>
                    </div>
      ...
      {/* CARRELLO */}
      <div style={{ position:'fixed', top:0, right: cartOpen ? 0 : -440, width:420, height:'100%', background:'#0F0F0F', borderLeft:'1px solid rgba(201,168,76,0.15)', zIndex:100, display:'flex', flexDirection:'column', transition:'right 0.35s cubic-bezier(0.4,0,0.2,1)' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'1.5rem', borderBottom:'1px solid rgba(201,168,76,0.1)' }}>
          ...
          <button onClick={() => { setCartOpen(false); setCheckingOut(false) }} style={{ background:'none', border:'1px solid rgba(201,168,76,0.2)', borderRadius:8, padding:'6px 10px', cursor:'pointer', color:'#888880', fontSize:16, transition:'all 0.2s' }}>✕</button>
        </div>
        {!checkingOut ? (
          <>
            <div style={{ flex:1, overflowY:'auto', padding:'1rem 1.5rem' }}>
              {cart.length === 0 ? (
                <div style={{ textAlign:'center', padding:'4rem 1rem', color:'#444440' }}>
                  <div style={{ fontSize:48, marginBottom:'1rem', opacity:0.5 }}>🛒</div>
                  <div style={{ fontSize:14 }}>Il carrello è vuoto</div>
                </div>
              ) : cart.map(c => (
                <div key={c.id} style={{ display:'flex', gap:12, padding:'14px 0', borderBottom:'1px solid rgba(255,255,255,0.05)', alignItems:'center' }}>
                  <div style={{ position:'relative', width:52, height:52, borderRadius:10, overflow:'hidden', flexShrink:0, background:'#1C1C1C', border:'1px solid rgba(201,168,76,0.1)' }}>
            ...
            {cart.length > 0 && (
              <div style={{ padding:'1.25rem 1.5rem', borderTop:'1px solid rgba(201,168,76,0.1)' }}>
                <div style={{ display:'flex', gap:8, marginBottom:12 }}>
                  ...
                  <button onClick={applyCoupon} className="btn-ghost" style={{ border:'none', padding:'9px 14px', borderRadius:10, fontSize:13, cursor:'pointer', fontWeight:500 }}>Applica</button>
                </div>
                {couponMsg && <div style={{ fontSize:13, marginBottom:10, color: discount > 0 ? '#4ade80' : '#f87171' }}>{couponMsg}</div>}
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                  <span style={{ fontSize:14, color:'#888880' }}>Subtotale</span>
                  <span style={{ fontSize:14, color:'#F5F5F0' }}>€{totalPrice.toFixed(2)}</span>
                </div>
                {discount > 0 && (
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    <span style={{ fontSize:14, color:'#4ade80' }}>Sconto {discount*100}%</span>
                    <span style={{ fontSize:14, color:'#4ade80' }}>-€{discountAmount.toFixed(2)}</span>
                  </div>
                )}
                {shippingCost > 0 && (
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
                    <span style={{ fontSize:14, color:'#888880' }}>🚚 Spedizione</span>
                    <span style={{ fontSize:14, color:'#F5F5F0' }}>€{shippingCost.toFixed(2)}</span>
                  </div>
                )}
                {shippingCost === 0 && cart.some(c => c.type === 'fisico') && (
                  <div style={{ fontSize:13, color:'#4ade80', marginBottom:8 }}>✅ Spedizione gratuita!</div>
                )}
                ...
                <button className="btn-gold" onClick={() => setCheckingOut(true)} style={{ width:'100%', border:'none', padding:14, borderRadius:24, fontSize:15, cursor:'pointer', fontWeight:600 }}>
          ...
          <div style={{ flex:1, padding:'1.5rem', overflowY:'auto' }}>
            {paid ? (
              <div style={{ textAlign:'center', padding:'3rem 1rem' }}>
                ...
                <div style={{ fontSize:14, color:'#888880', marginBottom:'1.5rem' }}>Controlla la tua email per la conferma.</div>
              ...
              <>
                ...
                <div style={{ marginBottom:'1.5rem', background:'#141414', borderRadius:12, padding:'1rem', border:'1px solid rgba(201,168,76,0.1)' }}>
                  ...
                  {cart.map(c => (
                    <div key={c.id} style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'#888880', padding:'4px 0' }}>
                      <span>{c.name} x{c.qty}</span>
                      <span>€{(Number(c.price) * c.qty).toFixed(2)}</span>
                    </div>
                  ))}
                  {discount > 0 && (
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'#4ade80', padding:'4px 0' }}>
                      <span>Sconto {discount*100}%</span>
                      <span>-€{discountAmount.toFixed(2)}</span>
                    </div>
                  )}
                  {shippingCost > 0 && (
                    <div style={{ display:'flex', justifyContent:'space-between', fontSize:13, color:'#888880', padding:'4px 0' }}>
                      <span>Spedizione</span><span>€{shippingCost.toFixed(2)}</span>
                    </div>
                  )}
                  <div style={{ display:'flex', justifyContent:'space-between', fontWeight:700, fontSize:16, marginTop:10, paddingTop:10, borderTop:'1px solid rgba(201,168,76,0.1)', color:'#C9A84C' }}>
                    <span>Totale</span><span>€{grandTotal.toFixed(2)}</span>
                  </div>
                </div>
                <PayPalButtons
      ...
      {/* WHATSAPP */}
      <a href="https://wa.me/393518435322" target="_blank" style={{ position:'fixed', bottom:'2rem', right:'2rem', background:'#25D366', color:'#fff', width:58, height:58, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, textDecoration:'none', boxShadow:'0 4px 24px rgba(37,211,102,0.35)', zIndex:90, transition:'transform 0.2s, box-shadow 0.2s' }}
        onMouseEnter={e => { e.currentTarget.style.transform='scale(1.12)'; e.currentTarget.style.boxShadow='0 8px 32px rgba(37,211,102,0.5)'; }}
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position:'fixed', bottom:'2rem', left:'50%', transform:'translateX(-50%)', background:'#141414', color:'#F5F5F0', padding:'12px 24px', borderRadius:24, fontSize:14, zIndex:200, border:'1px solid rgba(201,168,76,0.3)', boxShadow:'0 8px 32px rgba(0,0,0,0.5)', animation:'slideDown 0.3s ease' }}>
          {toast}
        </div>
      )}
      ...
      <footer style={{ borderTop:'1px solid rgba(201,168,76,0.1)', padding:'2.5rem', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:'1.5rem', justifyContent:'center', flexWrap:'wrap' }}>
          {[['Privacy Policy','/privacy'],['Termini e Condizioni','/termini'],['FAQ','/faq'],['Contattaci','/contatti']].map(([label, href]) => (

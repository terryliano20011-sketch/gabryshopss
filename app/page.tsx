  ...
  return (
    <div style={{ minHeight: '100vh', background: '#fff' }}>
      ...
      <div style={{ textAlign:'center', padding:'5rem 2rem 3rem', background:'#fff' }}>
        <h1 style={{ fontSize:48, fontWeight:700, letterSpacing:'-2px', marginBottom:'1rem', lineHeight:1.1 }}>Qualità senza<br />compromessi.</h1>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(240px, 1fr))', gap:'1.5rem', padding:'1.5rem 2rem 4rem', maxWidth:1200, margin:'0 auto' }}>
        {filtered.map(p => {
          ...
          return (
            <div key={p.id} style={{ border:'1px solid #eee', borderRadius:16, overflow:'hidden', background:'#fff', transition:'box-shadow 0.2s' }}>
      ...
      {/* OVERLAY */}
      {cartOpen && <div onClick={() => setCartOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.3)', zIndex:99 }} />}

      {/* CARRELLO */}
      <div style={{ position:'fixed', top:0, right: cartOpen ? 0 : -400, width:380, height:'100%', background:'#fff', borderLeft:'1px solid #eee', zIndex:100, display:'flex', flexDirection:'column', transition:'right 0.3s' }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'1.25rem 1.5rem', borderBottom:'1px solid #eee' }}>
          <span style={{ fontSize:17, fontWeight:600 }}>Carrello</span>
        ...
        {cart.length > 0 && (
          <div style={{ padding:'1.25rem 1.5rem', borderTop:'1px solid #eee' }}>
            ...
            <button style={{ width:'100%', background:'#1a1a1a', color:'#fff', border:'none', padding:14, borderRadius:24, fontSize:15, cursor:'pointer', fontWeight:500 }}>Procedi al pagamento</button>
          </div>
        )}
      </div>
